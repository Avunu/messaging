      ...
      <template #room-header="{ room }">
        <div class="custom-room-header">
          <div class="room-header-info">
            ...
            <span class="room-medium" :class="`medium-${room.communicationMedium?.toLowerCase()}`">
      ...
      <template #message="{ message }">
        <div class="custom-message" :class="{ 'message-failure': message.failure }">
          ...
          <div v-if="message.referenceDoctype" class="message-reference">
            <a 
              :href="`/app/${encodeDoctype(message.referenceDoctype)}/${message.referenceName}`"
              class="reference-link"
            >
        ...
        <div class="comm-panel" @click.stop>
          ...
          <div class="comm-panel-body">
            <div v-if="selectedMessage" class="comm-details">
              ...
              <div class="detail-row">
                <label>{{ __('Date') }}:</label>
                <span>{{ selectedMessage.date }} {{ selectedMessage.timestamp }}</span>
              </div>
              <div v-if="selectedMessage.referenceDoctype" class="detail-row">
                <label>{{ __('Reference') }}:</label>
                <a 
                  :href="`/app/${encodeDoctype(selectedMessage.referenceDoctype)}/${selectedMessage.referenceName}`"
                  target="_blank"
                >
                  {{ selectedMessage.referenceDoctype }}: {{ selectedMessage.referenceName }}
                </a>
              </div>
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const chatHeight = ref('calc(100vh - 120px)');
