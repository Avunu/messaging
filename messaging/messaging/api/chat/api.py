...
def _get_contact_from_identifier(
    medium: str, identifier: str
) -> dict[str, Any] | None:
    """
...
def _build_room_from_thread(
    thread: dict[str, Any], current_user_id: str
) -> Room:
    """
    ...
    last_message: LastMessage = {
        ...
        "senderId": thread.get("sender", identifier) if thread.get("sent_or_received") == "Received" else current_user_id,
        "timestamp": thread.get("communication_date", "").strftime("%H:%M") if thread.get("communication_date") else "",
    ...
    for comm in all_comms:
        ...
        if medium_type in ("SMS", "Phone"):
            ...
            if comm.get("sent_or_received") == "Received":
                ...
                identifier = comm.get("recipients", "").split(",")[0].strip() if comm.get("recipients") else ""
        ...
        if (
            comm.get("sent_or_received") == "Received"
            and not comm.get("seen")
        ):
            room_map[room_id]["unread_count"] = room_map[room_id].get("unread_count", 0) + 1

    # Convert to room objects and paginate
    all_rooms = [
        _build_room_from_thread(thread, current_user_id)
        for thread in room_map.values()
    ]
...
def get_messages(
    ...
    for idx, comm in enumerate(all_messages):
        ...
        if comm.get("sent_or_received") == "Sent":
            ...
            sender_id = comm.get("phone_no") or comm.get("sender") or identifier
        ...
        if comm.get("in_reply_to"):
            reply_comm = frappe.qb.from_(Communication).select(
                Communication.name,
                Communication.content,
                Communication.text_content,
                Communication.sender,
                ...
                Communication.message_id == comm["in_reply_to"]
            ...
            if reply_comm:
                ...
                reply_message = {
                    ...
                    "senderId": current_user_id if reply_comm[0].get("sent_or_received") == "Sent" else identifier,
                }
        ...
        message: Message = {
            ...
            "indexId": idx,
            "content": content,
            "username": comm.get("sender_full_name") or sender_id,
            ...
            "disableActions": comm.get("sent_or_received") == "Received",
            "disableReactions": True,
            "files": files if files else [],
            "communicationName": comm["name"],
            "communicationMedium": comm.get("communication_medium", "Email"),
            "sentOrReceived": comm.get("sent_or_received", "Received"),
        ...
        if reply_message:
            message["replyMessage"] = reply_message
...
def send_message(
    ...
    try:
        ...
        message: Message = {
            ...
            "disableReactions": True,
            "files": [],
            "communicationName": comm_doc.name,
            "communicationMedium": medium,
            "sentOrReceived": "Sent",
            "subject": subject,
